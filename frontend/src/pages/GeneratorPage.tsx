...
function Select({ label, children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & { label: string; children: React.ReactNode }) {
  return <label><span className="label">{label}</span><select className="input" {...props}>{children}</select></label>;
}
...
function Result({ prompt, editing, setEditing, onUpdate, onCopy, copied }: { prompt: Prompt; editing: boolean; setEditing: (value: boolean) => void; onUpdate: (changes: Partial<Prompt>) => Promise<void>; onCopy: () => Promise<void>; copied: boolean }) {
  ...
  return <div className="space-y-6"><Card className="overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b pb-4"><div><p className="eyebrow">Gotowe do użycia</p><h2 className="mt-1 text-xl font-black">Twój dopracowany prompt</h2></div><div className="flex gap-2"><Badge tone="green">Kompletność {prompt.quality_score}/100</Badge><Button variant="ghost" className="px-2.5" onClick={() => void onUpdate({ is_favorite: !prompt.is_favorite })} aria-label={prompt.is_favorite ? "Usuń z ulubionych" : "Dodaj do ulubionych"}><Heart size={18} fill={prompt.is_favorite ? "currentColor" : "none"} className={prompt.is_favorite ? "text-rose-500" : ""} /></Button></div></div><div className="mt-4">{editing ? <textarea value={content} onChange={(event) => setContent(event.target.value)} className="input min-h-[370px] font-mono text-xs leading-relaxed" /> : <pre className="max-h-[410px] overflow-auto whitespace-pre-wrap rounded-xl bg-slate-950 p-5 font-mono text-xs leading-6 text-slate-100">{prompt.content}</pre>}</div><div className="mt-4 flex flex-wrap gap-2">{editing ? <Button onClick={async () => { await onUpdate({ content }); setEditing(false); }}><Save size={16} />Zapisz zmiany</Button> : <Button variant="secondary" onClick={() => setEditing(true)}><Edit3 size={16} />Edytuj</Button>}<Button variant="secondary" onClick={() => void onCopy()}>{copied ? <Check size={16} className="text-emerald-500" /> : <Clipboard size={16} />}{copied ? "Skopiowano" : "Kopiuj"}</Button><Button variant="ghost" onClick={() => download(`/prompts/${prompt.id}/export?format=markdown`, "promptforge-prompt.md")}><Download size={16} />Markdown</Button><Button variant="ghost" onClick={() => download(`/prompts/${prompt.id}/export?format=pdf`, "promptforge-prompt.pdf")}><Download size={16} />PDF</Button><Link className="inline-flex items-center rounded-xl px-3 text-sm font-bold text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-500/10" to={`/app/prompty/${prompt.id}`}>Szczegóły</Link></div></Card><QualityReport prompt={prompt} /></div>;
}
